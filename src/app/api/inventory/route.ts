...
export async function GET() {
  ...
  const map = new Map<string, { item_name: string; spec: string | null; incoming: number; consumed: number }>()

  for (const r of reqRes.data || []) {
    if (r.purchase_quantity == null) continue
    const key = keyOf(r.item_name, r.spec)
    if (!map.has(key)) map.set(key, { item_name: r.item_name, spec: r.spec, incoming: 0, consumed: 0 })
    map.get(key)!.incoming += r.purchase_quantity
  }

  for (const c of consRes.data || []) {
    const key = keyOf(c.item_name, c.spec)
    if (!map.has(key)) map.set(key, { item_name: c.item_name, spec: c.spec, incoming: 0, consumed: 0 })
    map.get(key)!.consumed += c.quantity
  }

  const items = Array.from(map.values())
    .map(v => ({ ...v, stock: v.incoming - v.consumed }))
