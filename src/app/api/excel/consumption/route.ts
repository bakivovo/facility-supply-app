...
import { buildConsumptionWorkbook } from '@/lib/excelConsumption'
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 다음 달 1일 (존재하지 않는 날짜를 피하기 위해 lt 사용)
    ...
    const records  = data || []
    const workbook = buildConsumptionWorkbook(records, year, month)
    const buffer   = await workbook.xlsx.writeBuffer()

    return xlsxResponse(buffer, `소모내역_${year}년${month}월.xlsx`)
  } catch (err: any) {
    console.error('[excel/consumption] 엑셀 생성 오류:', err?.stack ?? err)
